package utils;

import java.io.*;
import java.util.HashMap;
import java.util.List;
import java.util.ArrayList;
import java.util.Map;

/** This class reads a directory's elements */
public class GenericDirectoryReader {

	public GenericDirectoryReader(){}
	
	/** this method reads a directory and returns a list with all texts (n lists of string) */ 
	public static List<List<String>> getDirectoryFiles(String directoryName){
		
		List<List<String>> directoryFiles = new ArrayList<List<String>>(); 
			
		File directory = new File(directoryName); // a directory also is a file
		
		if(!(directory.exists())){
			
			System.out.println("Look the paths, this error is about folder not exists");
			//TODO throws exception	
		}
		
		String filesInDirectory[] = directory.list(); // gets the files in the directory
		
		for(String f:filesInDirectory){ // iteration on all files in the directory
			//for(int i = 0 ; i<filesInDirectory.length ; i++){
			if(f.endsWith(".pol")){	// if the file that we're reading ends with .pol (to differentiate and ignore other file extensions like .jpg)
				directoryFiles.add(GenericReader.getFileContents(directoryName+f)); // calls the individual file reader, and adds the list(a file-> list of strings, one per line) to the list of lists (all files in directory)
			}
		}
		
		return directoryFiles;
	}
	
	/** this method reads a directory and returns a Map with filenames and all texts */ 
	public static Map<String, List<String>> getDirectoryFilesMap(String directoryName){
		
		Map<String,List<String>> directoryFiles = new HashMap<String,List<String>>(); 
			
		File directory = new File(directoryName); // a directory also is a file
		
		if(!(directory.exists())){
			
			System.out.println("Look the paths, this error is about folder not exists");
			//TODO throws exception	
		}
		
		String filesInDirectory[] = directory.list(); // gets the files in the directory
		
		for(String f:filesInDirectory){ // iteration on all files in the directory
			//for(int i = 0 ; i<filesInDirectory.length ; i++){
			if(f.endsWith(".pol")){	// if the file that we're reading ends with .pol (to differentiate and ignore other file extensions like .jpg)
				directoryFiles.put(f,GenericReader.getFileContents(directoryName+f)); // calls the individual file reader, and adds the list(a file-> list of strings, one per line) to the list of lists (all files in directory)
			}
		}
		
		return directoryFiles;
	}
	
}
